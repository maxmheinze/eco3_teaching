...
$$
\begin{aligned}
  y_i^{(0)}&=\beta_0+\beta_1x_i+u_i \\
  y_i^{(1)}&=y_i^{(0)}+\delta,
\end{aligned}
$$

where $\delta$ is the [**treatment effect**]{.col1}. 
...
We can find the [**treatment effect**]{.col1} $\delta=\mathrm{E}(y_i^{(1)}-y_i^{(0)}\mid x_i=c)$ like this:

$$
\begin{aligned}
   \delta & =\lim_{x_i\:\downarrow\:c} \mathrm{E}\big(y^{(0)}_i\mid x_i=c\big)-\lim_{x_i\:\uparrow\:{c}} \mathrm{E}\big(y^{(1)}_i\mid x_i=c\big)
   \\
          & = \lim_{x_i\:\downarrow\:c} \mathrm{E}\big(y_i\mid x_i=c\big) -\lim_{x_i\:\uparrow\:{c}}
   \mathrm{E}\big(y_i\mid x_i=c\big)
\end{aligned}
$$
