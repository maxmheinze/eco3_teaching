...
A **different framework** that has its strengths **elsewhere**: the [**Directed Acyclical Graphs (DAG)**]{.col2} framework.
...
* **Core idea**: treatment $D$ is assigned based on whether a [**running variable**]{.col2} $X$ crosses a known [**cutoff**]{.col2} $c$
$$
D_i = \begin{cases} 1 & \text{if } X_i \geq c \\ 0 & \text{if } X_i < c \end{cases}
$$
* **Intuition**: units just below and just above the cutoff are [**nearly identical**]{.col2} on average — the cutoff creates a [**local randomization**]{.col2}
* The causal effect is identified by the [**discontinuous jump**]{.col1} in $Y$ at $X = c$:
$$
\hat{\tau}_{RDD} = \lim_{x \downarrow c} E[Y_i | X_i = x] - \lim_{x \uparrow c} E[Y_i | X_i = x]
$$

## Regression Discontinuity Design (RDD) (2)
* **Two variants**:
  * [**Sharp RDD**]{.col2}: crossing the cutoff *perfectly* determines treatment
  * [**Fuzzy RDD**]{.col1}: crossing the cutoff only *increases the probability* of treatment → use cutoff as an **IV**
* [**Key assumption — continuity**]{.col1}: all other factors evolve [**smoothly through the cutoff**]{.col1} — no other discontinuity at $c$
* **Threats to validity**:
  * [**Manipulation**]{.col1}: units can precisely control $X_i$ to sort above/below cutoff 
  * [**Spillovers**]{.col1}: treatment affects units on both sides of the cutoff
* **Classic example**: financial aid cutoff — students just below a GPA threshold receive aid, those just above do not
...
* A [**unbalanced panel**]{.col2} has some missing data for at least one time period for at least one entity
...
$$
\text{Fatality\ Rate} = \beta_0 + \beta_1 \text{Beer\ Tax} + u_i
$$

## Example: Traffic Deaths and Alcohol Taxes (2)
* 1982 data:
$$
\text{Fatality\ Rate} = 2.01 + 0.15 \text{Beer\ Tax}
$$
* 1988 data:
$$
\text{Fatality\ Rate} = 1.86 + 0.44 \text{Beer\ Tax}
$$
...
&= \beta_0 + \beta_1 (\text{Beer\ Tax}_{i,1982} - \text{Beer\ Tax}_{i,1988}) \nonumber\\
...
* Cultural attitudes toward drinking driving affect the level of drunk driving and thus the traffic fatality rate in a state. 
...
* For instance, a variable could vary across entities (states) but do not change over time
...
* The variation in entity fixed effects comes from [**omitted variables**]{.col1} that, like $Z_i$ that vary across entities but **not over time**
...
* Difference from the "before and after" regression: uses only **1982 and 1988** data now we uses **all 7 years** → smaller standard error
...
* Just as the entity fixed effects regression model can be represented using $n- 1$ binary indicators
...
* We call this the **Two-Way Fixed Effects**

## Caveats of the Fixed Effects
...
* WVS: Covers 100+ccountries over multiple waves, focus on cultural values, beliefs, trust, religion, and social norms  
